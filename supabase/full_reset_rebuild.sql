...
BEGIN
  IF to_regclass('public.issues') IS NOT NULL THEN
    EXECUTE 'DROP TRIGGER IF EXISTS enforce_issue_update_permissions_trigger ON public.issues';
    EXECUTE 'DROP TRIGGER IF EXISTS update_issues_updated_at ON public.issues';
  END IF;
...
BEGIN
  ...
  IF public.has_role(auth.uid(), 'worker') THEN
    -- Worker can only change worker_status (+ updated_at managed by trigger)
    IF (NEW.status IS DISTINCT FROM OLD.status)
      ...
      OR (NEW.resolution_image_url IS DISTINCT FROM OLD.resolution_image_url)
      OR (NEW.duplicate_group_id IS DISTINCT FROM OLD.duplicate_group_id) THEN
